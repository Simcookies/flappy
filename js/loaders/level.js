...
import {createBackgroundLayer, createPipeLayer} from '../layers/background.js'
...
function setupCollision(levelSpec, level) {
  ...
  level.setCollosionGrid(collisionGrid);
}
...
function setupBackgrounds(levelSpec, level, backgroundSprites) {
  levelSpec.layers.forEach(layer => {
    ...
    const pipeLayer = createPipeLayer(backgroundGrid, backgroundSprites);
    level.comp.layers.push(backgroundLayer, pipeLayer);
  });
}
...
export function createLevelLoader(entityFactory) {
  return function loadLevel(name) {
    ...
    .then(([levelSpec, backgroundSprites]) => {
      // levelSpec: JSON format for level;
